@php
    $currentFilter = request()->get('chart_filter_period', 'last_6_months');
    $userGrowthChartData = $user_growth_data['data'] ?? [];
    $userGrowthChartLabels = $user_growth_data['labels'] ?? [];
@endphp
...
<script>
    (function () {
        const chartDataPayload = {
            labels: @json($userGrowthChartLabels),
            data: @json($userGrowthChartData),
            filter: @json($currentFilter),
        };
        ...
        function initUserGrowthChart() {
            ...
            const currentFilter = chartDataPayload.filter;
            const userGrowthLabels = chartDataPayload.labels;
            const userGrowthData = chartDataPayload.data;
            ...
            if (currentFilter === 'last_6_months') {
                ...
                chartCategories = userGrowthLabels.length ? userGrowthLabels.slice(0, currentMonth + 1) : thisYearLabels;
                chartData = userGrowthData.length ? userGrowthData.slice(0, currentMonth + 1) : [230, 280, 350, 310, 285, 390].slice(0, currentMonth + 1);
            } else if (currentFilter === 'last_year') {
                chartCategories = userGrowthLabels.length ? userGrowthLabels : ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC'];
                chartData = userGrowthData.length ? userGrowthData : [190, 220, 270, 330, 320, 410, 390, 380, 360, 300, 340, 370];
                ...
                chartCategories = userGrowthLabels.length >= 30 ? userGrowthLabels.slice(-30) : last30DaysLabels;
                chartData = userGrowthData.length >= 30 ? userGrowthData.slice(-30) : last30DaysData;
                ...
                chartCategories = userGrowthLabels.length >= 7 ? userGrowthLabels.slice(-7) : last7DaysLabels;
                chartData = userGrowthData.length >= 7 ? userGrowthData.slice(-7) : last7DaysData;
                ...
                chartCategories = userGrowthLabels.length >= currentDay ? userGrowthLabels.slice(0, currentDay) : thisMonthLabels;
                chartData = userGrowthData.length >= currentDay ? userGrowthData.slice(0, currentDay) : thisMonthData;
            } else {
                chartCategories = userGrowthLabels.length ? userGrowthLabels : ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC'];
                chartData = userGrowthData.length ? userGrowthData : [120, 270, 340, 415, 320, 560, 420, 380, 365, 390, 400, 450];
            }
            ...
            window.userGrowthChartInstance = new ApexCharts(chartElement, options);
            window.userGrowthChartInstance.render();
        }

        document.addEventListener('DOMContentLoaded', initUserGrowthChart);
        document.addEventListener('livewire:navigated', initUserGrowthChart);
    })();
</script>
